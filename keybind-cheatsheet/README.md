...
| Field | Value |
| --- | --- |
| ID | `mindset/keybind-cheatsheet` |
| Entries | Data service: `data`; bar widget: `keybinds`; panel: `cheatsheet` |

`mindset` is the plugin's publisher namespace, not your local Linux
...
```sh
noctalia msg panel-toggle mindset/keybind-cheatsheet:cheatsheet
```
...
```ini
bind=SUPER,F1,spawn,noctalia msg panel-toggle mindset/keybind-cheatsheet:cheatsheet
```

Hyprland classic configuration:

```ini
bind = SUPER, F1, exec, noctalia msg panel-toggle mindset/keybind-cheatsheet:cheatsheet
```

Niri:

```kdl
Mod+F1 { spawn "noctalia" "msg" "panel-toggle" "mindset/keybind-cheatsheet:cheatsheet"; }
```
...
```sh
noctalia msg plugin mindset/keybind-cheatsheet:data all refresh
```
...
```sh
noctalia msg plugin mindset/keybind-cheatsheet:keybinds focused toggle
noctalia msg plugin mindset/keybind-cheatsheet:keybinds focused refresh
```

For parser development, run the fixture suite inside Noctalia:

```sh
noctalia msg plugin mindset/keybind-cheatsheet:data all self-test
```
